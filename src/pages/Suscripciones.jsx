// src/pages/Suscripciones.jsx — DocVias v22
...
export default function Suscripciones() {
  ...
  // Cargar historial de facturas DocVias
  useEffect(() => {
    if (!tenantId) return
    const q = query(
  ...
  // Cargar config DocVias (para mostrar datos de contacto/soporte)
  useEffect(() => {
  ...
  const pagarAhora = async () => {
    ...
    if (!link) {
      toast.error('Configura el Payment Link de DocVias en Admin → Sistema → Configuración')
      return
    }
  ...
  const activarSuscripcionAuto = async () => {
    ...
    if (!priceId) {
      toast.error('Configura los Price IDs de Stripe en Admin → Sistema → Configuración DocVias')
      return
    }
    setCargandoSub(true)
    try {
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl">
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-gray-400 mt-0.5">DocVias · {tenant?.nombre ?? ''}</p>
      </div>
      ...
      {config?.rfc && (
        <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 mt-4">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">
            Datos fiscales de DocVias (emisor)
          </p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4"
          ...
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl"
            ...
            <p className="text-sm text-gray-500 mb-5">
              Elige cómo quieres pagar tu suscripción DocVias cada mes.
            </p>
