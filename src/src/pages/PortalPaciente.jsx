...
// ── Hook: obtener datos del paciente por email ────────────
function usePacientePortal() {
  const [state, setState] = useState({
    user: null, paciente: null, tenantId: null,
    citas: [], documentos: [], medicamentos: [], loading: true,
  })
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async user => {
      ...
      // Buscar paciente por email en todos los tenants
      try {
        const tenantsSnap = await getDocs(collection(db, 'tenants'))
        let pacienteEncontrado = null
        let tenantIdEncontrado = null

        for (const tenantDoc of tenantsSnap.docs) {
          const pacientesSnap = await getDocs(
            query(
              collection(db, `tenants/${tenantDoc.id}/pacientes`),
              where('email', '==', user.email)
            )
          )
          if (!pacientesSnap.empty) {
            pacienteEncontrado = { id: pacientesSnap.docs[0].id, ...pacientesSnap.docs[0].data() }
            tenantIdEncontrado = tenantDoc.id
            break
          }
        }
  ...
  if (!user) return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      <div className="bg-white rounded-2xl p-8 w-full max-w-sm shadow-xl text-center">
        <h1 className="text-2xl font-bold text-slate-800 mb-1">MediDesk</h1>
        <p className="text-sm text-gray-500 mb-6">Portal del paciente</p>
        <p className="text-sm text-gray-600">
          Inicia sesión para ver tu expediente
        </p>
        <a href="/login-paciente"
          className="mt-4 inline-block px-6 py-2.5 bg-teal-600 text-white
